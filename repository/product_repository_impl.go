...
func (repository *ProductRepositoryImpl) Save(ctx context.Context, tx *sql.Tx, product domain.Product) domain.Product {
	SQL := "INSERT INTO products(name,description,category_id) VALUES ($1,$2,$3) RETURNING id"
	row := tx.QueryRowContext(ctx, SQL, product.Name, product.Description, product.CategoryID)
...
func (repository *ProductRepositoryImpl) FindByID(ctx context.Context, tx *sql.Tx, productID int) (domain.Product, error) {
	SQL := "SELECT id,name,description,category_id FROM products WHERE id = $1"

	product := domain.Product{}

	err := tx.QueryRowContext(ctx, SQL, productID).Scan(&product.ID, &product.Name, &product.Description, &product.CategoryID)
...
func (repository *ProductRepositoryImpl) FindAll(ctx context.Context, tx *sql.Tx) []domain.Product {
	SQL := "SELECT id,name,description,category_id FROM products"
	...
	for rows.Next() {
		product := domain.Product{}
		err := rows.Scan(&product.ID, &product.Name, &product.Description, &product.CategoryID)
		helper.PanicIfError(err)
